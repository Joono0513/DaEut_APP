...
class UpdateScreen extends StatefulWidget {
  ...
  const UpdateScreen({super.key, required this.serviceRequest});
...
class _UpdateScreenState extends State<UpdateScreen> {
  ...
  void _loadExistingData() {
    _titleController.text = widget.serviceRequest.serviceName;
    _priceController.text = widget.serviceRequest.servicePrice.toString();
    _contentController.text = widget.serviceRequest.serviceContent;
    _selectedCategories = widget.serviceRequest.serviceCategory.split(',');
    // 썸네일과 이미지 로드 로직 추가 필요
  }
    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        var serviceRequest = ServiceRequest(
          ...
          thumbnailPath: thumbnailPath.isNotEmpty ? thumbnailPath : widget.serviceRequest.thumbnailPath,
          filePaths: imagePaths.isNotEmpty ? imagePaths : widget.serviceRequest.filePaths,
        );

        var url = "http://10.0.2.2:8080/reservation/update";
